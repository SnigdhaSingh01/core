   ...
   def clean_email(self, *args, **kwargs):
     ...
     # domain = formEmail.split('@')[1]
      #domain_list = ["ssct.edu.ph"]
        if '@' not in formEmail:
            raise forms.ValidationError("Email format is not valid. It should be in the format xyz@something.com")

        username, domain = formEmail.split('@')
        if '.' not in domain or domain.split('.')[-1].lower() not in ['com']:
            raise forms.ValidationError("Please enter a valid email domain (e.g., .com)")
        ...
        if self.instance.pk is None: 
            ...
            if db_email != formEmail:  # There have been changes
                if Account.objects.filter(email=formEmail).exists():
                    raise forms.ValidationError("The given email is already registered")
...
class UpdateProfileForm(FormSettings):
   ...
   def clean_email(self, *args, **kwargs):
      ...
      if not formEmail.endswith('@kmn.com'):
            raise forms.ValidationError("Please enter a valid email ending with @kmn.com")
